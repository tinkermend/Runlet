...
### Changed
- **precompiled 重试退避验证补强（Task 5）**：`tests/backend/test_run_check_job.py` 新增退避调用序列回归，用 `monkeypatch` 捕获 `app.jobs.run_check_job.anyio.sleep` 调用并断言指数退避序列；同时在测试内固定 `_PRECOMPILED_JITTER_MS=10`，保证 jitter 可控且断言稳定。
