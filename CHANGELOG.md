...
### Added
- 修复 chat-check references 契约漂移：`template-slots.md` 槽位名对齐后端模板注册（`field_equals_exists` 改为 `field/operator/value`，`count_gte` 改为 `min_count` 并补充“至少 N 条”映射），`decision-rules.md` 增补直执行高置信锚点（`rank_score`/`sample_count`/前两名 `rank_score` 差值的派生判断），`result-format.md` 增补定位信息需“请求上下文 + 结果”联合归纳说明。
