...
- `template_code`
- `template_version`
- `supported_carriers`（V1 严格限制: `table/list`，不新增 `detail` carrier）
- `required_slots`
- `assertion_contract`
- `compile_strategy`
...
2. 默认返回 `2-3` 个可点击候选配置。
3. 候选排序规则：**历史命中成功率优先**。
   - 统计粒度：`template_code + system_id + page_asset_id`
   - 冷启动回退：若样本不足（< 20 次），回退 `intent_alias confidence`，再回退最近使用
...
3. 状态筛选与标签存在
4. 分页/排序后数据断言保持成立
5. 常见异常态识别（无权限/配额不足/业务错误提示）
   - 实现边界：异常态识别由“模板断言结果 + runner 失败分类”协同输出，不单独新增并行断言引擎

说明：V1 不新增 `detail` carrier，且不包含“详情页可达与关键字段存在”检查；该能力明确放入 V2。
...
说明：当前阶段采用临时统一口径进行验收，避免统计歧义：

- 统计窗口：最近 14 天，且样本量不少于 200 次请求
- 若最近 14 天样本不足 200 次，则滚动累积到最近 400 次请求
- 后续再根据运行数据固化为长期 7/30 天双窗口口径
